...
async function getUser(env,req){try{const t=getToken(req);if(!t)return null;const uid=await env.SESSIONS.get(`session:${t}`);if(!uid)return null;return await env.DB.prepare('SELECT id,name,email,role,plan,plan_expires_at,twofa_enabled,twofa_type,cf_account_email FROM users WHERE id=?').bind(uid).first();}catch{return null;}}
...
export async function onRequestGet({ request, env }) {
  try {
    ...
    const cfRow = await env.DB.prepare('SELECT cf_global_api_key,cf_account_id FROM users WHERE id=?').bind(user.id).first();
